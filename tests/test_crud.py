...
class TestTeacherCRUD:
    ...
    def test_get_or_create_teacher_new(self, session):
        ...
        teacher = crud.get_or_create_teacher(session, telegram_id=111, name="Новый")
...
class TestStudentCRUD:
    ...
    def test_create_student(self, session, teacher):
        """Создание студента."""
        student = crud.create_student(session, teacher.id, "Тестов Тест Тестович")
    ...
    def test_get_all_students_by_teacher(self, session, teacher, students):
...
class TestSubjectStudentCRUD:
    ...
    def test_add_student_to_subject_duplicate(self, session, subject, students):
        ...
        link2 = crud.add_student_to_subject(session, subject.id, students[0].id)
    ...
    def test_get_students_by_subject(self, session, subject_with_students, students):
    ...
    def test_remove_student_from_subject(self, session, subject_with_students, students):
        ...
        remaining = crud.get_students_by_subject(session, subject_with_students.id)
        assert len(remaining) == 2
    
    def test_count_students_in_subject(self, session, subject_with_students):
        """Подсчёт студентов в дисциплине."""
        count = crud.count_students_in_subject(session, subject_with_students.id)
...
class TestAttendanceCRUD:
    ...
    def test_set_attendance_update(self, session, subject_with_students, students):
        ...
        crud.set_attendance(session, students[0].id, subject_with_students.id, today, True)
    ...
    def test_get_attendance_by_subject_and_date(self, session, attendance_data):
        ...
        result = crud.get_attendance_by_subject_and_date(session, subject.id, test_date)
        
        assert len(result) == 3
        assert all(v is True for v in result.values())  # Все присутствовали 01.11
    ...
    def test_get_student_attendance_by_subject(self, session, attendance_data):
        ...
        att = crud.get_student_attendance_by_subject(session, students[0].id, subject.id)
