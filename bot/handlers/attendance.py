...
def get_date_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(f"📅 Сегодня ({format_date(today)})", callback_data=f"att_date_{today.isoformat()}"),
        ],
        [
            InlineKeyboardButton(f"📅 Вчера ({format_date(yesterday)})", callback_data=f"att_date_{yesterday.isoformat()}"),
        ],
        [
            InlineKeyboardButton("📆 Другая дата...", callback_data="att_date_custom"),
        ],
        [
...
def get_students_attendance_keyboard(
    ...
    keyboard.append([
        InlineKeyboardButton("✅ Все присутствуют", callback_data=f"att_all_present_{subject_id}_{attendance_date.isoformat()}"),
    ])
    keyboard.append([
        InlineKeyboardButton("❌ Все отсутствуют", callback_data=f"att_all_absent_{subject_id}_{attendance_date.isoformat()}"),
    ])
    keyboard.append([
        InlineKeyboardButton("💾 Готово", callback_data=f"att_done_{subject_id}_{attendance_date.isoformat()}"),
    ])
    keyboard.append([
        InlineKeyboardButton("◀️ Выбрать другую дату", callback_data=f"att_select_date_{subject_id}"),
    ])
...
async def attendance_date_custom(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("◀️ Назад", callback_data=f"att_select_date_{subject_id}")],
    ])
...
async def attendance_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Сохраняем в БД
        crud.set_attendance(session, student_id, subject_id, attendance_date, new_status)
...
async def attendance_all_present(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        for student in students:
            crud.set_attendance(session, student.id, subject_id, attendance_date, True)
...
async def attendance_all_absent(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        for student in students:
            crud.set_attendance(session, student.id, subject_id, attendance_date, False)
...
async def attendance_done(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.info(
            ...
            subject.name, format_date(attendance_date), present_count, len(students)
        )
        
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📅 Другая дата", callback_data=f"att_select_date_{subject_id}")],
            [InlineKeyboardButton("📚 Другая дисциплина", callback_data="menu_attendance")],
...
def get_attendance_conversation_handler() -> ConversationHandler:
    ...
    return ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(attendance_all_present, pattern=r"^att_all_present_\d+_\d{4}-\d{2}-\d{2}$"),
            CallbackQueryHandler(attendance_all_absent, pattern=r"^att_all_absent_\d+_\d{4}-\d{2}-\d{2}$"),
            CallbackQueryHandler(attendance_done, pattern=r"^att_done_\d+_\d{4}-\d{2}-\d{2}$"),
        ],
        states={
            AttendanceStates.SELECT_DATE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, attendance_date_input),
                CallbackQueryHandler(attendance_select_date, pattern=r"^att_select_date_\d+$"),
            ],
        },
        fallbacks=[
