...
def get_students_pool_keyboard(teacher_id: int, session) -> InlineKeyboardMarkup:
    ...
    keyboard.append([
        InlineKeyboardButton("➕ Добавить студента", callback_data="pool_student_add"),
    ])
    keyboard.append([
        InlineKeyboardButton("📋 Добавить список", callback_data="pool_student_bulk"),
    ])
    keyboard.append([
...
async def pool_student_add_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.info("Создан студент: %s (teacher_id=%s)", student.full_name, teacher.id)
...
async def pool_student_bulk_names(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.info("Создано %s студентов (teacher_id=%s)", len(students), teacher.id)
...
async def pool_student_view(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✏️ Изменить ФИО", callback_data=f"pool_student_edit_{student_id}"),
            ],
            [
                InlineKeyboardButton("🗑 Удалить", callback_data=f"pool_student_delete_{student_id}"),
            ],
            [
                InlineKeyboardButton("◀️ К списку студентов", callback_data="menu_students"),
            ],
        ])
...
async def pool_student_edit_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("❌ Отмена", callback_data=f"pool_student_view_{student_id}")]
        ])
...
async def pool_student_delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if attendances:
            warnings.append(f"• Будут удалены {len(attendances)} записей посещаемости")
        ...
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ Да, удалить", callback_data=f"pool_student_delete_yes_{student_id}"),
                InlineKeyboardButton("❌ Отмена", callback_data=f"pool_student_view_{student_id}"),
            ],
        ])
...
def get_students_conversation_handler() -> ConversationHandler:
    ...
    return ConversationHandler(
        entry_points=[
            CallbackQueryHandler(students_pool_menu, pattern="^menu_students$"),
            CallbackQueryHandler(pool_student_add_start, pattern="^pool_student_add$"),
            CallbackQueryHandler(pool_student_bulk_start, pattern="^pool_student_bulk$"),
            CallbackQueryHandler(pool_student_view, pattern=r"^pool_student_view_\d+$"),
            CallbackQueryHandler(pool_student_edit_start, pattern=r"^pool_student_edit_\d+$"),
            CallbackQueryHandler(pool_student_delete_confirm, pattern=r"^pool_student_delete_\d+$"),
            CallbackQueryHandler(pool_student_delete_yes, pattern=r"^pool_student_delete_yes_\d+$"),
        ],
        states={
            StudentStates.WAITING_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, pool_student_add_name),
                CallbackQueryHandler(students_pool_menu, pattern="^menu_students$"),
            ],
            StudentStates.WAITING_BULK_NAMES: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, pool_student_bulk_names),
                CallbackQueryHandler(students_pool_menu, pattern="^menu_students$"),
            ],
            StudentStates.WAITING_NEW_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, pool_student_edit_name),
                CallbackQueryHandler(pool_student_view, pattern=r"^pool_student_view_\d+$"),
            ],
        },
        fallbacks=[
