...
from bot.utils.stats import get_subject_stats, get_teacher_overall_stats
...
def get_period_keyboard(subject_id: int) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [InlineKeyboardButton(
            "📅 За всё время", callback_data=f"stats_period_{subject_id}_all")],
        ...
        [InlineKeyboardButton(
            "📆 Указать период...", callback_data=f"stats_period_{subject_id}_custom")],
    ...
    try:
        ...
        if not subjects:
            ...
            keyboard = [
                ...
                [InlineKeyboardButton(
                    "— Или выберите дисциплину —", callback_data="noop")],
            ]
...
async def stats_overall(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton(
            "📅 За всё время", callback_data="stats_overall_period_all")],
        [InlineKeyboardButton(f"📅 Последняя неделя ({format_date(week_ago)} — {format_date(today)})",
...
async def stats_overall_period_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton("📈 График по дисциплинам",
                              callback_data="stats_overall_chart")],
        [InlineKeyboardButton(
            "📅 Другой период", callback_data="stats_overall")],
        [InlineKeyboardButton("◀️ Назад", callback_data="menu_stats")],
    ]
...
async def stats_subject(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if not dates:
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton(
                    "✏️ Отметить посещаемость", callback_data=f"att_select_date_{subject_id}")],
                [InlineKeyboardButton("◀️ Назад", callback_data="menu_stats")],
            ])
            await query.edit_message_text(
...
async def stats_period_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if period == "week":
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "◀️ Отмена", callback_data=f"stats_subject_{subject_id}")]
        ])
        await query.edit_message_text(
            text=(
                "📆 <b>Укажите период</b>\n\n"
...
async def show_subject_stats(query, context, subject_id: int, date_from, date_to, period_text: str) -> int:
    ...
    if stats.get("total_dates", 0) == 0:
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(
                "📅 Другой период", callback_data=f"stats_subject_{subject_id}")],
            [InlineKeyboardButton("◀️ Назад", callback_data="menu_stats")],
        ])
        await query.edit_message_text(
    ...
    if len(students) > 8:
        ...
        for st in students[-3:]:
            ...
            lines.append(
                f"{idx}. {st['student_name']}: {format_percentage(st['percentage'])}")
    elif len(students) > 5:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "📊 По датам", callback_data=f"stats_chart_dates_{subject_id}"),
            InlineKeyboardButton(
                "👥 По студентам", callback_data=f"stats_chart_students_{subject_id}"),
        ],
        [InlineKeyboardButton(
            "📅 Другой период", callback_data=f"stats_subject_{subject_id}")],
        [InlineKeyboardButton("◀️ Назад", callback_data="menu_stats")],
...
async def stats_chart_dates(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("◀️ Назад к статистике",
                              callback_data=f"stats_subject_{subject_id}")],
    ])
...
def get_stats_conversation_handler() -> ConversationHandler:
    ...
    return ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(stats_overall_period_selected,
                                 pattern=r"^stats_overall_period_\w+$"),
            CallbackQueryHandler(stats_overall_chart,
                                 pattern="^stats_overall_chart$"),
            CallbackQueryHandler(
                stats_subject, pattern=r"^stats_subject_\d+$"),
            CallbackQueryHandler(stats_period_selected,
                                 pattern=r"^stats_period_\d+_\w+$"),
            CallbackQueryHandler(
                stats_chart_dates, pattern=r"^stats_chart_dates_\d+$"),
            CallbackQueryHandler(stats_chart_students,
                                 pattern=r"^stats_chart_students_\d+$"),
            CallbackQueryHandler(noop_callback, pattern="^noop$"),
        ],
        states={
            WAITING_STATS_DATE_FROM: [
                MessageHandler(filters.TEXT & ~filters.COMMAND,
                               stats_date_from_input),
                CallbackQueryHandler(
                    stats_subject, pattern=r"^stats_subject_\d+$"),
            ],
