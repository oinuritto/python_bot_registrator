...
def get_period_keyboard(export_type: str, subject_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [InlineKeyboardButton(
            "📅 За всё время", callback_data=f"{prefix}_all")],
        ...
        [InlineKeyboardButton("📆 Указать период...",
...
async def do_export(update: Update, context, export_type: str, subject_id: int | None,
        ...
        if export_type == "subject" and subject_id:
            ...
            if not students:
                keyboard = InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton(
                        "◀️ Назад", callback_data="menu_export")],
                ])
                await query.edit_message_text(
    ...
    try:
        if export_type == "subject" and subject_id:
            file_data = create_attendance_report(
                subject_id, date_from, date_to)

            timestamp = datetime.now().strftime("%Y%m%d_%H%M")
            safe_name = "".join(
                c for c in subject.name if c.isalnum() or c in " _-").strip()[:30]
            ...
            file_data = create_all_subjects_report(
                teacher.id, date_from, date_to)
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📊 Экспортировать ещё",
...
def get_export_conversation_handler() -> ConversationHandler:
    ...
    return ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(export_select_period,
                                 pattern=r"^export_select_period_"),
        ...
        states={
            WAITING_DATE_FROM: [
                MessageHandler(filters.TEXT & ~filters.COMMAND,
                               export_date_from_input),
                CallbackQueryHandler(export_menu, pattern="^menu_export$"),
            ],
            WAITING_DATE_TO: [
                MessageHandler(filters.TEXT & ~filters.COMMAND,
                               export_date_to_input),
                CallbackQueryHandler(export_menu, pattern="^menu_export$"),
            ],
        },
        fallbacks=[
