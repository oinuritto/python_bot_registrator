...
def create_calendar(
    ...
    # Дни недели
    days_row = [InlineKeyboardButton(day, callback_data="noop") for day in DAYS_RU]
    ...
    # Быстрые кнопки
    quick_row = [
        InlineKeyboardButton("📅 Сегодня", callback_data=f"{callback_prefix}_today_{subject_id}"),
        InlineKeyboardButton("📅 Вчера", callback_data=f"{callback_prefix}_yesterday_{subject_id}"),
    ]
    keyboard.append(quick_row)
    
    # Кнопка "Назад"
    keyboard.append([
        InlineKeyboardButton("◀️ Назад", callback_data=f"menu_attendance")
    ])
